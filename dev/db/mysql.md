...
10.将文本文件导入数据表中（excel与之相同）load data infile "tables.txt" into table table_name;  
...
20.修改数据表中的数据：update table_name set col_name=new_value where id=1;  

例如：update bbs set name="tom" where name="jack";

21.增加一个字段：alter table table_name add column field_name datatype not null default "1";  
...
23.删除一个字段：alter table table_name drop field_name;  

例如：alter table bbs drop tel;

24.修改字段的数据类型：

alter table table_name modify id int unsigned;//修改列id的类型为int unsigned   alter table table_name change id sid int unsigned;//修改列id的名字为sid，而且把属性修改为int unsigned  
...
45.percent_rank() over(partition by init_date,interval_type order by drawdown)    数据占的百分占
